...
echo "=== TEST RESPONSE TIMEOUT X (MDI / REAL CMD DEMAND) ==="
echo "started=$(date '+%F %T')"
echo "observe_seconds=$OBSERVE_SECONDS sample_interval=$SAMPLE_INTERVAL inject_seconds=$INJECT_SECONDS"
echo "ASSUMPTION: AXIS is idle, enabled, and you will issue a REAL X move immediately after this script arms the path"

echo "[1/7] clear injector bits (X only)"
...
echo "[2/7] force neutral semantic state (X only)"
halcmd sets motion-req-x FALSE
sleep 0.1

echo "[3/7] reset watchdog X from idle"
...
echo "[4/7] arm response-timeout path (X)"
...
echo
echo ">>> NOW issue a REAL X move in AXIS/MDI immediately."
echo ">>> Example: a short X move that changes xpos-cmd but does not rely on feedback moving."
echo ">>> Press ENTER here right after you trigger the move."
read -r

echo "--- POST-TRIGGER PREFLIGHT X ---"
halcmd getp motion_wd_x.armed
halcmd getp motion_wd_x.motion-req
...
echo "[5/7] observe X after real command demand"
echo "------------------------------------------------------------"
...
echo "[7/7] expected success criteria"
echo "- response-pending should become TRUE"
echo "- response-tmr should grow"
echo "- response-timeout should become TRUE"
echo "- first-fault-code should be 1 (unless another persistent fault dominates later)"
