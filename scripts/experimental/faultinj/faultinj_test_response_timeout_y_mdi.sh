...
echo "=== TEST RESPONSE TIMEOUT Y (MDI / REAL CMD DEMAND) ==="
echo "started=$(date '+%F %T')"
echo "observe_seconds=$OBSERVE_SECONDS sample_interval=$SAMPLE_INTERVAL inject_seconds=$INJECT_SECONDS"
echo "ASSUMPTION: AXIS is idle, enabled, and you will issue a REAL Y move immediately after this script arms the path"

echo "[1/7] clear injector bits (Y only)"
...
echo "[2/7] force neutral semantic state (Y only)"
halcmd sets motion-req-y FALSE
sleep 0.1

echo "[3/7] reset watchdog Y from idle"
...
echo "[4/7] arm response-timeout path (Y)"
...
echo
echo ">>> NOW issue a REAL Y move in AXIS/MDI immediately."
echo ">>> Example: small Y move (G91 G0 Y5)."
echo ">>> Press ENTER here right after you trigger the move."
read -r

echo "--- POST-TRIGGER PREFLIGHT Y ---"
halcmd getp motion_wd_y.armed
halcmd getp motion_wd_y.motion-req
...
echo "[5/7] observe Y after real command demand"
echo "------------------------------------------------------------"
...
echo "- first-fault-code should be 1"
